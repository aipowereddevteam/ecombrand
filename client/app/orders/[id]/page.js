...
export default function UserOrderDetails({ params }) {
    const { id } = use(params);
                    ...
                    <div className="md:col-span-2 space-y-6">
                        <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
                            ...
                            <div className="relative pl-4">
                                {/* Vertical Line */}
                                <div className="absolute left-[23px] top-2 bottom-4 w-0.5 bg-border/50"></div>
                                ...
                                {steps.map((step, index) => {
                                    ...
                                    return (
                                        <div key={step} className="relative flex gap-4 mb-8 last:mb-0 group">
                                            {/* Icon Circle */}
                                            <div className={`w-8 h-8 rounded-full flex items-center justify-center z-10 border-2 transition-all duration-300 transform group-hover:scale-110
                                                ${isCompleted
                                                    ? 'bg-primary border-primary text-primary-foreground shadow-md shadow-primary/20'
                                                    : 'bg-background border-muted text-muted-foreground'}
                                            `}>
                                                {isCompleted ? <CheckCircle size={14} /> : <div className="w-2 h-2 rounded-full bg-muted-foreground/30"></div>}
                                            </div>
                                            ...
                                            <div className="pt-1">
                                                ...
                                                {entry && (
                                                    <div className="text-xs text-muted-foreground mt-1 flex items-center gap-1.5 bg-muted/30 px-2 py-1 rounded-md w-fit">
                                                        <Clock size={10} />
