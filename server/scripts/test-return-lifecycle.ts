...
async function runTest() {
    ...
    try {
        ...
        const returnReq = await ReturnRequest.create({
            ...
            refundAmount: 1000,
            status: 'QC_Passed', // Simulate passed QC
            auditLog: [{ status: 'QC_Passed', timestamp: new Date(), note: 'Simulated QC' }]
        });
        console.log('ReturnRequest created:', returnReq._id);

        // 5. Trigger Refund Job
        // The worker is running in the main server process (npm start).
        // By adding to the queue here, the MAIN SERVER should pick it up if it's connected to the same Redis.
        ...
        // Since worker is external (in server process), we just poll DB.
        console.log('Waiting for worker to update DB...');
        let retries = 15;
        let success = false;
        ...
        if (!success) {
            console.error('\nFAILED: Timeout waiting for ReturnRequest update. Ensure server is running and RefundWorker is active.');
        } else {
            // Verify Transaction
