...
export default function AdminQCPanel() {
    ...
    const fetchRequests = async () => {
        ...
        try {
            const { data } = await axios.get(`${process.env.NEXT_PUBLIC_API_URL || 'http://0.0.0.0:5000/api/v1'}/admin/requests?status=Requested`, {
            ...
            const allRes = await axios.get(`${process.env.NEXT_PUBLIC_API_URL || 'http://0.0.0.0:5000/api/v1'}/admin/requests`, {
                 headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleQCAction = async (id: string, action: 'QC_Passed' | 'QC_Failed', notes: string) => {
        ...
        try {
            await axios.patch(`${process.env.NEXT_PUBLIC_API_URL || 'http://0.0.0.0:5000/api/v1'}/admin/qc`, {
                returnRequestId: id,
                status: action,
                notes,
